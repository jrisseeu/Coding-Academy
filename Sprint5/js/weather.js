...
function getTheForecast() {
    return __awaiter(this, void 0, void 0, function () {
        var location, apiUrl, response, myObj, nbrDays, headerData, tblInfo, j;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    location = document.getElementById("location").value;
                    apiUrl = "https://api.weather.gov/gridpoints/" + location + "/25,45/forecast";
                    return [4 /*yield*/, fetch(apiUrl.toString())];
                case 1:
                    response = _a.sent();
                ...
                case 2:
                    myObj = _a.sent();
                    nbrDays = myObj.properties.periods.length;
                    headerData = createTableHeader();
                    tblInfo = "";
                    for (j = 0; j < nbrDays; j++) {
                        tblInfo += createTableData(myObj.properties.periods[j].name, myObj.properties.periods[j].shortForecast, myObj.properties.periods[j].temperature, myObj.properties.periods[j].icon);
                    }
